...
<div class="modal-header">
    ...
    <h4 class="modal-title modaltitletextnew">Edit Property</h4>
</div>
                ...
                <div id="p_info" class="tab-pane fade in active">
                    ...
                    <div class="col-md-10">
                        <div class="parmentaddress parmentaddressnew">
                            ...
                            if ($model->status == 1) {
                                ?>
                                <?= $form->field($model, 'property_name')->hiddenInput(['maxlength' => true, 'placeholder' => 'Property Name*', 'disabled' => 'false'])->label(false) ?>
                                <?php
                            } else {
                                ?>
                            ...
                            <div id="no_of_beds" style="display:none;">
                                ...
                                <div id="newFields">
                                    ...
                                    foreach ($roomsOne as $key => $data) {
                                        ...
                                        echo '<label> Room ' . ($key + 1) . ' </label> <input type="text" placeholder="Beds*" value="' . $roomsChildOne . '" name="Properties[beds][]" class="form-control isNumber">';
                                    }
                                    ?>
                            ...
                            if ($model->status == 1) {
                                ?>
                                <?= $form->field($model, 'address_line_1')->hiddenInput(['maxlength' => true, 'id' => 'searchTextField', 'placeholder' => 'Address line 1*'])->label(false) ?>

                                <?= $form->field($model, 'address_line_2')->hiddenInput(['maxlength' => true, 'placeholder' => 'Address line 2*'])->label(false) ?>
                                <?php
                            } else {
                                ?>
                            ...
                            if ($model->status == 1) {
                                ?>
                                <input type="hidden" id="location" class="form-control" placeholder="Location*" aria-required="true" aria-invalid="false">

                                <?= $form->field($model, 'lat')->hiddenInput(['maxlength' => true, 'placeholder' => 'Latitude*', 'class' => 'form-control isLocation'])->label(false); ?>
                                <?= $form->field($model, 'lon')->hiddenInput(['maxlength' => true, 'placeholder' => 'Longitude*', 'class' => 'form-control isLocation'])->label(false); ?>
                                <?php
                            } else {
                                ?>
                                <false class="field-properties required">
                                <label class="control-label">Location</label><input type="text" id="location" class="form-control" placeholder="Location*" aria-required="true" aria-invalid="false"><div class="help-block"></div>
                                </false>
...
?>
                            ...
                            <div style="padding-top:20px;" class="row pull-right col-lg-9 text-right">
                                <div class="col-lg-6">
                            ...
                            <div style="padding-top:20px;" class="row pull-right col-lg-8">
                                ...
                                <div class="col-lg-4">
                                    <form method="POST" id="formfaclities">
                                        ...
                                        <?= Html::Button('Nextcvc', ['id' => 'save_features', 'class' => 'btn btn savechanges_submit_contractaction']) ?>
                                    </form>
                ...
                <div id="p_image" class="tab-pane fade">
                    ...
                    <div class="row" style= "width:52%; padding: 10px;">
                        ...
                        <div class="parmentaddress parmentaddressnew">
                            ...
                            $form = ActiveForm::begin(['id' => 'property_agreement', 'action' => 'addagreement?id=' . $model->id,
                                        'options' => [
                                            ...
                                            'data-href' => 'addagreementajax?id=' . $model->id,
                                            'class' => 'form-inline',
                                        ],
                                        'fieldConfig' => [
                                            'template' => "{label}{input}{error}",
                            ...
                            if ($model->status == 1) {
                                ?>
                                <?= $form->field($modelPropertyAgreements, 'agreement_type')->dropDownList([1 => 'Normal Leasing', 2 => 'Guaranteed Leasing', 3 => 'Co-living'], ['prompt' => 'Select Agreement Type', 'style' => 'display:none'])->label(false); ?>
                                <?php
                            } else {
                                ?>
                            ...
                            <?= $form->field($modelPropertyAgreements, 'notice_peroid')->textInput(['maxlength' => true, 'placeholder' => 'Notice period*'])->label('Notice Period(Months)') ?>
                            <?= $form->field($modelPropertyAgreements, 'min_contract_peroid')->textInput(['maxlength' => true, 'placeholder' => 'Minimum contract period*', 'class' => 'form-control isNumber1'])->label('Minimum Contract Period(Months)') ?>
                            ...
                            if ($model->status == 1) {
                                ?>
                                <?= $form->field($modelPropertyAgreements, 'contract_start_date')->hiddenInput(['maxlength' => true, 'placeholder' => 'Contract start date*', 'class' => 'datepicker'])->label(false) ?>
                                <?= $form->field($modelPropertyAgreements, 'contract_end_date')->hiddenInput(['maxlength' => true, 'placeholder' => 'Contract end date*', 'class' => 'datepicker'])->label(false) ?>
                                <?php
                            } else {
                                ?>
                            ...
                            <div style="padding-top:20px;" class="row pull-right col-lg-12 col-sm-12">
                                <div class="col-lg-4 col-sm-4">
                                    <button type="button" class="btn savechanges_submit_previous activate_property"  style="<?= $model->status == 1 ? 'font-size:20px;padding:12.5px' : 'font-size:22px;padding:11px' ?>" data-status="<?= ($model->status == 1) ? '0' : '1' ?>"><?= ($model->status == 1) ? 'Deactivate Property' : 'Activate Property' ?></button>

                                </div>
                                <div class="col-lg-4 col-sm-4">
                                    <?= Html::submitButton('Save', ['class' => 'btn btn savechanges_submit_contractaction', 'id' => 'saveContract']) ?>
                                </div>
                                <div class="col-lg-4 col-sm-4">
                                    <button type="button" class="btn savechanges_submit cancel_confirm" data-dismiss="modal">cancel</button>
...
<script>
    ...
    $(document).on("click", "#save_features", function (e) {
        ...
        $.ajax({
            url: 'saveattributes',
            type: 'POST',
            data: {property_id: $("#attri_poperty_id").val(), attributes: $("#attributes").val(), property_map: str, _csrf: csrfToken},
            success: function (data) {
