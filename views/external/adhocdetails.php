...
<div id="page-wrapper">
    <div class="row">
        <h1>Ad-hoc Charge For Tenant</h1>
        <?php
        ...
        ?>
        <?php
        $tenant_name = Yii::$app->userdata->getFullNameById($model->tenant_id);
        ?>
        <div class="form-group">
            <label class="control-label" >Tenant Name</label>
            <input type="text" value="<?= $tenant_name ?>" class="form-control" placeholder="Tenant Name*" readonly>
            ...
            <div class="lorembox" id="dataComments">
                ...
                if ($comments) {
                    foreach ($comments as $comment) {
                        ?>
                        <p><?= $comment->message; ?>,
                            <span>  -<?= Yii::$app->userdata->getFullNameById($comment->user_id); ?> on  <?= date('d-M-Y', strtotime($comment->created_date)); ?> At  <?= date('h:i a', strtotime($comment->created_date)); ?></span>
                        </p>
                        <?php
                    }
                } else {
            ...
            <div class="images_list">
                ...
                if (count($modelAttachments) != 0) {
                    foreach ($modelAttachments as $key => $value) {
                        ?>
                        <div>
                            <a href="<?php echo Url::home(true) . $value->url; ?>" download><?php echo Url::home(true) . $value->url; ?></a>
                        </div>
                        <?php
                    }
                }
        ...
        <?= $form->field($modelPayment, 'created_date')->textInput(['maxlength' => true, 'placeholder' => 'Transaction Id*', 'class' => 'form-control datepicker'])->label('Date Of Payment'); ?>
        <?php
        ...
        <div class="row advisorbox">
            ...
            <div class="col-md-6">
                <button type="button" class="btn savechanges_submit" data-dismiss="modal" onclick="cancel_confirm();">Cancel</button>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#add_comment', function (e) {
        ...
        $.ajax({
            ...
            error: function (data) {
                hideLoader();                 // alert(data.responseText);
                alert('Failure')
...
<script>
    function cancel_confirm() {
        ...
        if (r == true) {
            window.location.replace("<?php echo Url::home(true); ?>external/adhocrequests");
        }
    }
</script>
